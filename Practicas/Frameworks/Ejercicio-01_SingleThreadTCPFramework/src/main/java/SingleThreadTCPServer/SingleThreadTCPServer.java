...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            String inputLine;
            boolean freno = false;
            while (((inputLine = in.readLine()) != null)&&(!freno)) {
                mensajeRecibidoLog(clientSocket, inputLine);
                freno = cerrarConexion(inputLine);
                handleMessage(inputLine, out);
            }
            conexionCerradaLog(clientSocket);
        } catch (IOException e) {
    ...
    protected void mensajeRecibidoLog(Socket clienteSocket, String mensaje) {
        System.out.println("Received message: " + mensaje + " from "
                + clienteSocket.getInetAddress().getHostAddress() + ":" + clienteSocket.getPort());
    }

    protected boolean cerrarConexion(String mensaje){
        return mensaje.equalsIgnoreCase("");
    }

    protected void conexionCerradaLog(Socket clienteSocket){
        System.out.println("Connection closed with " + clienteSocket.getInetAddress().getHostAddress() + ":"
